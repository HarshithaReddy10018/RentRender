...
def split_data(df):
    ...
    # Remove ID columns and unnecessary text columns if they exist
    drop_cols = [
        col for col in [
            "id",
            "title",
...
def split_X_data(df, drop_cols=None):
...
if __name__ == "__main__":
    ...
    X_train.to_csv(
        r"C:\2nd year odd\ML\PythonProject\data\preprocessed_train.csv",
        index=False
    )

    X_test.to_csv(
        r"C:\2nd year odd\ML\PythonProject\data\preprocessed_test.csv",
        index=False
    )
